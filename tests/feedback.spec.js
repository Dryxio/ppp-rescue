...
test('feedback sends explicit answers only, never recovery data',async({page})=>{
 const sent=[];await page.route(endpoint,async route=>{expect(route.request().headers().referer).toBe('http://127.0.0.1:4178/');sent.push(route.request().postDataJSON());await route.fulfill({json:{success:'true'}});});
