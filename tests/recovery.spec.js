...
test('responsive at common widths',async({page},testInfo)=>{for(const width of [375,768,1024,1440]){await page.setViewportSize({width,height:900});await page.goto('/');expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);}await page.setViewportSize({width:375,height:812});if(testInfo.project.name==='chromium')await page.screenshot({path:'docs/mobile.png',fullPage:true});});
